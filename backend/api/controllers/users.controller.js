...
export default class UsersController {
  ...
  static async apiRegisterUser(req, res) {
    if (!(req.body.password === req.body.verifyPassword))
      return res.status(400).json({ error: "verifyPassword failed" }); // check that the passwords entered match
    ...
    try {
      //check if there's already an account associated with email or username
      const emailExists = await UsersDAO.getUserByEmail(req.body.email);
      const usernameExists = await UsersDAO.getUserByUsername(
        req.body.username
      );
      if (emailExists)
        return res
          .status(400)
          .json({ error: "email already associated with an accout" });
      if (usernameExists)
    ...
    try {
      ...
      res.cookie("jwt", token, { httpOnly: true, maxAge: maxAge * 1000 }).json({
